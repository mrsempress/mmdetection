...
class PointTransform(object):
    ...
    1. rescale points according to image size
    2. flip points (if needed)
    """

    def __call__(self, points, pose, img_shape, scale_factor, flip=False):
        if len(points) == 0:
            return points, pose
        gt_points = points * scale_factor.reshape((1, 1, -1))
        if flip:
            w = img_shape[1]
            flipped = gt_points.copy()
            gt_points[:, :, 0] = w - flipped[:, [2, 1, 0, 5, 4, 3], 0]
            gt_points[:, :, 1] = flipped[:, [2, 1, 0, 5, 4, 3], 1]
            flip_pose_map = np.array([0, 7, 6, 5, 4, 3, 2, 1])
            pose[pose >= 0] = flip_pose_map[pose[pose >= 0]]

        return gt_points, pose
